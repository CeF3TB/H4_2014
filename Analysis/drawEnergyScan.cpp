...
int main( int argc, char* argv[] ) {
  ...
  */
  ...
  TFile* fileMean = TFile::Open(Form("analysisTrees_%s/Reco_%s.root", tag.c_str(),runs[2].c_str()));
  TTree* treeMean = (TTree*)fileMean->Get("recoTree");
 
  //and for CeF3:
  TF1* energyfuncC = fitSingleElectronPeak( outputdir, runs[2], treeMean );

  float energy491C= beamEnergy[2];
  ...
  for( unsigned i=0; i<runs.size(); ++i ) {
    ...
    gr_resp_vs_energy->SetPointError( i, energyErr/1000., meanErr/1000.);
 ...
 for( unsigned i=0; i<simulation.size(); ++i ) {
  ...
  TH2D* h2_axes = new TH2D( "axes", "", 10, 0., xMax/1000., 10, 0., 2650000 );
...
leg0->SetTextSize(0.038);
  ...
  leg0->AddEntry(f1_lines,Form("Offset = %.1f\n #pm %.1f\n MeV",f1_lines->GetParameter(0), f1_lines->GetParError(0) ),"L");
 ...
 ///////////////////////RESOLUTION///////////////////////////////
  TH2D* h2_axes2 = new TH2D( "axes", "", 100, -0.0, 155. , 10, 0.0, 30 );
 ...
 TF1 *fun= new TF1("fun",  "sqrt([0]*[0]/x+[1]*[1]/(x*x))",0.1, 155);
 ...
 TF1 *fun1= new TF1("fun1",  "sqrt([0]*[0]/x+[1]*[1])",0.0005, 155);
...
ResoStruct getResponseResolutionMC( const std::string& outputdir, TTree* tree, const std::string& name, float energyS ) {
  ...
  TH1D* h1 = new TH1D( name.c_str(), "", 500, energyS/3.-10, energyS/2.3+100 );
  tree->Project( name.c_str(),"cef3_corr[0]+cef3_corr[1]+cef3_corr[2]+cef3_corr[3]" );

   // tree->Project( name.c_str(),"cef3_corr[0]+cef3_corr[1]+cef3_corr[2]+cef3_corr[3]","(isSingleEle_scintFront==1 &&  nHodoClustersY==1 )" );

 TF1* f1 = new TF1( Form("f1_%s", name.c_str()), "gaus",  energyS/6.-100, energyS/2.+10 );
...
TF1* fitSingleElectronPeak( const std::string& outputdir, const std::string& name, TTree* tree ) {
  ...
  TH1D* h1 = new TH1D(histoName.c_str(), "", 10000, 0., 2400000.);

  tree->Project( histoName.c_str(), "cef3[0]+cef3[1]+cef3[2]+cef3[3]");

  TF1* f1 = new TF1( Form("gaus_%s", name.c_str()), "gaus", 50000., 2400000.);
  //  TF1* f1 = new TF1( Form("gaus_%d", i), "gaus", 400., 1200.);
      f1->SetParameter(0, 300000.);
   f1->SetParameter(1, 2500000.);
